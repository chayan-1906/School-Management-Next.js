...
const renderRow = (role: string, {id, lesson: {subject, class: examOfClass, teacher}, dueDate}: AssignmentList) => {
    return (
...
async function AssignmentsPage({searchParams}: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
    ...
    return (
        <div className={'flex-1 rounded-md p-4 m-4 mt-0'}>
            ...
            {/** LIST */}
            <Table columns={columns({role})} data={assignments} renderRow={(item) => renderRow(role, item)}/>
